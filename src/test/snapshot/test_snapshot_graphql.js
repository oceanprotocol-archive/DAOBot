...
const granularVoting_proposalIPFSHash = "QmWQQqJbdLYTW5sU3Khif1gxmeEXoujRzwzHP5uJiPwX9h"
...
const uni_sushi_bancor_proposalIPFSHash = "QmQgfxvLqz88pL3ByK6U82bxezCU8MAiCSgxtTTCoR3fWm"
...
describe('Snapshot GraphQL test', () => {
    ...
    it('Validates scores from Single-Batch Voting', async () => {
        ...
        let votes = []
        await getProposalVotesGQL(singleBatchVoting_proposalIPFSHash)
            .then((result) => {
        ...
        const voterScores = await getVoterScores(strategy, voters, singleBatchVoting_blockHeight)
    ...
    it('Validates scores from Y/N granular voting', async () => {
        ...
        let votes = []
        await getProposalVotesGQL(granularVoting_proposalIPFSHash)
            .then((result) => {
                votes = result.data.votes
            })
        ...
        const voterScores = await getVoterScores(strategy, voters, granularVoting_blockHeight)
        const reducedVoterScores = reduceVoterScores(strategy, votes, voterScores)
    ...
    it('Validates scores from UniV2/Sushi/Bancor', async () => {
        ...
        let votes = []
        await getProposalVotesGQL(uni_sushi_bancor_proposalIPFSHash)
            .then((result) => {
                votes = result.data.votes
            })
        ...
        const voterScores = await getVoterScores(strategy, voters, uni_sushi_bancor_blockNumber)
        const reducedVoterScores = reduceVoterScores(strategy, votes, voterScores)
