...
const main = async () => {
    ...
    const estBlockHeight = Math.floor(blockNumber + ((targetTimestamp - curTimestamp) / avgBlockTime))

    assert(estBlockHeight === calcTargetBlockHeight(blockNumber, targetTimestamp, avgBlockTime), 'Estimated and calculated block heights do not match');
